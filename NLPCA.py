...
class NLPCA(TransformerMixin, BaseEstimator):
  ...
  def __init__(self, n_components=None, *, max_iteration=100, batch=None, verbose=0, opti_algoritm='sgd',
    ...
    # 参数说明
      ...
      max_iteration                  - 网络的最大迭代次数，分为 100 个 epoch 进行。
      batch                          - 整数或 None。每次梯度更新的样本数量。
                                      如果未指定，默认值为 32。如果数据以数据集、生成器或 
                                      keras.utils.Sequence 实例的形式提供，请勿指定 batch_size，
                                      因为这些形式会自动生成批量数据。
      verbose                        - 0, 1 或 2。详细模式：
                                      0 = 每 100 个 epoch 显示一次部分进度；
                                      1 = 完整的进度条；
                                      2 = 每个 epoch 显示一行日志。
                                      注意，当将日志记录到文件时，进度条并不特别有用，因此在生产环境中
                                      （如非交互模式运行时）推荐使用 verbose=2。
      opti_algoritm                  - 字符串（优化器名称）或网络优化器实例。
                                      默认值：'adam'
      n_components                   - 请求的非线性主成分的数量。
      pre_pca                        - {'True','False'} 默认值：'False'。PCA 预处理，使用前 n 个主成分，
                                      其中 n 是输出单元的数量。
      n_pre_pca                      - PCA 预处理中使用的前 n 个主成分的数量。
                                      默认值：0.1 / max(std(data),[],2)
      pre_unit_variance              - {'True','False'} 默认值：'False'。单位方差归一化。
      units_per_Hidden_layer         - 每个隐藏层中的神经元数量，不包括输入层、输出层和瓶颈层。
                                      默认值：1 层，包含 2 + (2 * k) 个神经元，其中 k 为请求的非线性主成分数量。
      weight_initialization          - 默认值：'None'，权重随机初始化。
                                      备选值：'linear'，权重线性初始化。
      weight_decay                   - 'True'：开启权重衰减（默认）。
                                      'False'：关闭权重衰减。
      weight_decay_coefficient       - 权重衰减系数，取值范围为 0 到 1。默认值：0.01。
      pre_scaling                    - True：限制数据集中的最大标准差，以便网络在初始阶段保持线性范围。
                                      默认值：如果未指定 'scaling_factor'，将标准差设置为 0.1。
      scaling_factor                 - 0.xx：缩放倍数。

      function_activation_linear     - 线性层的线性激活函数。
                                      默认值：'linear'。
      function_activation_nonlinear  - 非线性层的非线性激活函数。
                                      默认值：'tanh'。
      random_state                   - 控制将数据拆分为随机训练和测试子集时的洗牌操作。
                                      传递一个整数以便在多次函数调用中生成可重复的输出。
      callbacks                      - 如果为 True，使用训练期间的回调列表。
      callbacks_path                 - 应用于训练期间的回调文件路径。
    """
